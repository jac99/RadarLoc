...
def in_train_split(pos):
    ...
    mask = (dist > TEST_REGION_RADIUS).all(axis=1)
    return mask
...
def read_poses(poses_filepath: str, readings_filepath: str, sensor_code: str, pose_time_tolerance: float = 1.):
    ...
    assert sensor_code == 'R' or sensor_code == 'L', f"Unknown sensor code: {sensor_code}"
    ...
    all_poses = np.zeros((n, 4, 4), dtype=np.float64)       # 4x4 pose matrix
    ...
    for ndx, pose in enumerate(txt_poses):
        ...
        all_poses[ndx] = np.array([[float(temp[1]), float(temp[2]), float(temp[3]), float(temp[4])],
    ...
    for ndx, ts in enumerate(all_timestamps):
        ...
        if (sensor_code == 'L') and (ts in FAULTY_POINTCLOUDS):
            continue
