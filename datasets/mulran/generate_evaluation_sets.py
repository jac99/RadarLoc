# Evaluation sets for Mulran dataset.
...
if __name__ == '__main__':
    ...
    for map_sequence, query_sequence in sequences:
        ...
        # Downsampled radar images for our RadarLoc method
        resolution = '384_128'
        test_set = generate_evaluation_set(args.dataset_root, map_sequence, query_sequence,
                                           min_displacement=args.min_displacement, map_sensor='R', query_sensor='R',
                                           polar_folder=f'polar_{resolution}', dist_threshold=args.dist_threshold)

        pickle_name = f'test_R_{map_sequence}_{query_sequence}_{resolution}.pickle'

        file_path_name = os.path.join(args.dataset_root, pickle_name)
        test_set.save(file_path_name)

        # Downsampled radar images for ScanContext
        resolution = '120_40'
        test_set = generate_evaluation_set(args.dataset_root, map_sequence, query_sequence,
                                           min_displacement=args.min_displacement, map_sensor='R', query_sensor='R',
                                           polar_folder=f'polar_{resolution}', dist_threshold=args.dist_threshold)
        ...
        # Evaluation sets based on LiDAR
        test_set = generate_evaluation_set(args.dataset_root, map_sequence, query_sequence,
                                           min_displacement=args.min_displacement, map_sensor='L',
                                           query_sensor='L', dist_threshold=args.dist_threshold)
